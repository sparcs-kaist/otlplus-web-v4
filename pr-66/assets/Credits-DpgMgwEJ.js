...
`,Re=({options:e=["option1","option2","option3"],zindex:t=10,disabledOptions:c=[],setSelectedOption:h,selectedOption:i,isDetail:g=!0})=>{const d=q(),[r,o]=n.useState(!1),p=n.useRef(null),[f,v]=n.useState({top:0,left:0,width:0}),[y,S]=n.useState(0);n.useEffect(()=>{if(e.length>0){const u=document.createElement("canvas").getContext("2d");if(!u)return;u.font="14px Pretendard, sans-serif";const m=Math.max(...e.map(b=>u.measureText(b).width));S(m+50)}},[e]),n.useEffect(()=>{if(p.current){const s=p.current.getBoundingClientRect();v({top:s.top,left:s.left,width:Math.max(s.width,y)})}},[r,y]);const x=n.useRef(null);return n.useEffect(()=>{const s=()=>{if(p.current){const u=p.current.getBoundingClientRect();v({top:u.top,left:u.left,width:u.width})}};return s(),window.addEventListener("resize",s),()=>{window.removeEventListener("resize",s)}},[r]),n.useEffect(()=>{if(x.current&&e.length>0&&i>=0){const s=x.current.children[i];if(s){const m=s.getBoundingClientRect().top-x.current.getBoundingClientRect().top;x.current&&(x.current.scrollTop=m)}}},[i,r,e]),L(de,{isExpand:r,width:y,children:[L(ae,{ref:p,onClick:()=>{o(!r)},children:[e[i],l(Z,{onClick:()=>{o(!r)},styles:{padding:0},children:l(H,{size:24,color:d.colors.Highlight.default,onClick:()=>{},children:r?l(te,{}):l(re,{})})})]}),r&&l(ue,{ref:x,top:f.top,left:f.left,width:f.width,zindex:t,isDetail:g,children:e.map((s,u)=>{const m=c.includes(u),b=i==u;return l(le,{onClick:()=>{h(u),o(!1)},disabled:m,children:l(fe,{disabled:m,selected:b,children:s})},u)})})]})};function Ee(e,t,c){const{i18n:h}=F(),{infinites:i,headers:g={},enabled:d=!0,staleTime:r=1/0,gcTime:o=300*1e3,initialOffset:p=0,limit:f,select:v=a=>a,iterate:y=()=>-1}=c,[S,x]=n.useState(void 0),[s,u]=n.useState(null),m=n.useRef(v),b=n.useRef(i);n.useEffect(()=>{m.current=v},[v]),n.useEffect(()=>{b.current=i},[i]);const w=V(e,K(t)),P=ee({queryKey:[t,s],queryFn:async({pageParam:a=0})=>{const R=p+a*f,{data:E}=await X.request({method:e,url:"/api/v2"+t,params:{...s,offset:R,limit:f},headers:g});return E},initialPageParam:0,getNextPageParam:(a,R)=>{const E=y(a);if(E!==-1)return E>0?R.length:void 0;for(const B of b.current)if(a[B].length<f)return;return b.current.length>0?R.length:void 0},retry:1,staleTime:r,gcTime:o,enabled:d&&(s!==null||w.safeParse({})?.success===!0)});return n.useEffect(()=>{const a=P.data?.pages;if(!a||a.length===0){x(void 0);return}if(b.current.length===0){const B=a[a.length-1];x(m.current(B));return}const E={...a[a.length-1]};b.current.forEach(B=>{const k=a.flatMap(W=>W[B]);E[B]=k}),x(m.current(E))},[P.data]),n.useEffect(()=>{P.isEnabled&&P.refetch()},[h.resolvedLanguage]),{query:P,setParams:u,data:S}}const C=$(D)`
    text-decoration: none;
`,he=$.a`
    text-decoration: none;
`,Se=()=>{const{t:e}=F(),t=U("laptop");return L(N,{direction:"column",gap:12,align:"center",justify:"stretch",children:[l(T,{color:"Text.placeholder",type:"Bigger",children:"OTL PLUS"}),L(N,{direction:"row",gap:t?10:12,justify:"space-between",children:[l(C,{to:"/makers",children:l(T,{color:"Text.placeholder",type:t?"Normal":"Big",children:e("credits.credits")})}),l(T,{color:"Text.placeholder",type:t?"Normal":"Big",children:"|"}),l(C,{to:"/license",children:l(T,{color:"Text.placeholder",type:t?"Normal":"Big",children:e("credits.license")})}),l(T,{color:"Text.placeholder",type:t?"Normal":"Big",children:"|"}),l(C,{to:"/privacy-policy",children:l(T,{color:"Text.placeholder",type:t?"Normal":"Big",children:e("credits.privacyPolicy")})})]}),l(he,{href:"mailto:otlplus@sparcs.org",children:l(T,{color:"Text.placeholder",type:"Big",children:"otlplus@sparcs.org"})}),l(T,{color:"Text.placeholder",type:"Big",children:"Ⓒ 2026, SPARCS OTL TEAM"})]})};export{Se as C,te as E,Re as S,re as a,we as b,Ee as u};
//# sourceMappingURL=Credits-DpgMgwEJ.js.map
