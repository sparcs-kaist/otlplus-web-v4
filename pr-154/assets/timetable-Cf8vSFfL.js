...
`,ze=({selectedCourseId:e,selectedProfessorId:t})=>{let{t:n}=A(),[r,i]=(0,W.useState)(!1),{query:a,setParams:o,data:s}=w(`GET`,`/reviews`,{infinites:[`reviews`],limit:20}),{ref:d,inView:p}=C({threshold:0});return(0,W.useEffect)(()=>{i(!1);let n=setTimeout(()=>{o({mode:`default`,courseId:e??void 0,professorId:t??void 0}),i(!0)},1e3);return()=>clearTimeout(n)},[e,t]),(0,W.useEffect)(()=>{p&&a.hasNextPage&&!a.isFetchingNextPage&&a.fetchNextPage()},[p,a]),!s||!r?c(z,{}):l(u,{children:[c(f,{direction:`row`,gap:0,justify:`center`,align:`center`,style:{width:`100%`},children:c(Ie,{direction:`row`,gap:0,justify:`space-between`,align:`center`,children:[[y(s?.averageGrade,s?.reviews.length),n(`common.grade`)],[y(s?.averageLoad,s?.reviews.length),n(`common.load`)],[y(s?.averageSpeech,s?.reviews.length),n(`common.speech`)]].map(([e,t],n)=>l(Le,{direction:`column`,gap:0,align:`center`,children:[c(M,{type:`Bigger`,color:`Text.default`,children:e}),c(M,{type:`Smaller`,color:`Text.default`,children:t})]},n))})}),c(Re,{}),s?.reviews.map(e=>c(re,{review:e,linkToDictionary:!0},e.id)),a.hasNextPage&&c(z,{ref:d})]})},Be=d(f)`
    width: 100%;
    height: 100%;
    overflow-x: hidden;
    overflow-y: auto;
...
`,Je=({addLectures:e,removeLectures:t,onMobileModalClose:n,currentTimetableId:r,timetableLectures:i})=>{let a=g(),o=s(),{t:d}=A(),{user:m,status:_}=h(),v=L(`tablet`),y=G(e=>e.selectedLectures),b=G(e=>e.hoveredLectures),x=G(e=>e.year),C=G(e=>e.semesterEnum),w=y.length===1?y[0]:b.length===1?b[0]:null,[T,D]=(0,W.useState)([]),{query:k,setParams:N}=S(`GET`,`/users/${m?.id}/wishlist`,{enabled:_===`success`&&x!==-1});(0,W.useEffect)(()=>{x!==-1&&N({year:x,semester:C})},[x,C,r]),(0,W.useEffect)(()=>{k.data&&D(k.data.courses.flatMap(e=>e.lectures).map(e=>e.id))},[k.data]);let{requestFunction:ee}=S(`PATCH`,`/users/${m?.id}/wishlist`,{onSuccess:()=>{a.invalidateQueries({queryKey:[`/users/${m?.id}/wishlist`]})}}),F=(0,W.useRef)(null),I=e=>{let t={syy:String(x),smtDivCd:String(C),subjtCd:e.code,syllabusOpenYn:`0`};return`https://erp.kaist.ac.kr/com/lgin/SsoCtr/initExtPageWork.do?link=estblSubjt&params=${btoa(JSON.stringify(t))}`},te=t=>{if(!i)return;let n=i.some(e=>we(e.classes,t.classes));if(Ee({status:_,currentTimetableId:r??null,hasOverlap:n})){n&&alert(d(`timetable.addLectureConflict`));return}e([t]),p(`Add Lecture to Timetable`,{lectureId:t.id,lectureCode:t.code,courseName:t.name,timetableId:r,source:`LectureDetail`})},R=(e,t)=>{if(_===`idle`)return;let n=e?`delete`:`add`;p(`Update Wishlist`,{action:n,lectureId:t,source:`LectureDetail`}),ee({lectureId:t,mode:n})};return c(Be,{direction:`column`,gap:12,align:`center`,justify:!w&&!y.length?`center`:`start`,children:y.length>1?l(Ke,{direction:`column`,gap:16,align:`stretch`,justify:`flex-start`,children:[l(M,{type:`Bigger`,color:`Text.default`,children:[y.length,d(`timetable.numSelected`)]}),y.map(e=>{let t=se(o.colors.Tile.TimeTable.default),n=t[e.courseId%t.length];return l(qe,{direction:`row`,gap:12,align:`center`,children:[c(O,{size:14,color:n,children:c(U,{})}),l(f,{direction:`column`,gap:4,flex:`1 1 auto`,children:[l(M,{type:`BigBold`,color:`Text.default`,children:[e.name,` `,e.subtitle]}),l(M,{type:`Small`,color:`Text.dark`,children:[e.professors[0]?.name,` `,e.professors.length>1?`${d(`common.professors.over`)} ${e.professors.length-1}${d(`common.professors.people`)} `:` `,`| `,e.department.name,` | `,e.credit,d(`common.credit`)]})]})]},e.id)})]}):w?l(u,{children:[l(He,{direction:`column`,gap:2,align:`center`,justify:`center`,children:[l(f,{direction:`row`,align:`center`,gap:8,justify:v?`space-between`:`center`,style:{width:`100%`},children:[v&&c(`div`,{style:{width:20}}),c(M,{type:`Bigger`,color:`Text.default`,children:w.name+w.subtitle}),v&&c(O,{size:20,onClick:n,color:o.colors.Text.default,children:c(j,{})})]}),c(M,{type:`Big`,color:`Text.default`,children:w.code+` `+(w.classNo===``?``:`(${w.classNo})`)})]}),l(f,{direction:`row`,gap:8,justify:`flex-end`,style:{width:`100%`},children:[c(We,{to:`/dictionary?courseId=${w.courseId}`,children:c(M,{type:`Normal`,color:`Highlight.default`,style:{cursor:`pointer`},children:d(`header.dictionary`)})}),c(Ge,{href:I(w),target:`_blank`,children:c(M,{type:`Normal`,color:`Highlight.default`,style:{cursor:`pointer`},children:d(`header.syllabus`)})})]}),c(Ve,{direction:`column`,gap:10,align:`center`,children:c(Fe,{selectedLecture:w})}),c(Ve,{ref:F,direction:`column`,gap:10,children:c(ze,{selectedCourseId:w.courseId,selectedProfessorId:w.professors[0]?.id??-1})}),v&&l(Ue,{direction:`row`,gap:12,justify:`flex-end`,children:[_===`success`&&l(P,{onClick:()=>{R(T.includes(w.id),w.id),n&&n()},children:[c(O,{size:15,children:c(B,{})}),c(M,{type:`NormalBold`,children:T.includes(w.id)?d(`timetable.removeFromWishlist`):d(`timetable.addToWishlist`)})]}),(r!=null||_===`idle`)&&(i?.some(e=>e.id===w.id)?l(P,{type:`selected`,onClick:()=>{t&&t(w.id),n&&n()},children:[c(O,{size:15,children:c(j,{})}),c(M,{type:`NormalBold`,children:d(`timetable.removeFromTimetable`)})]}):l(P,{type:`selected`,onClick:()=>{te(w),n&&n()},children:[c(O,{size:15,children:c(H,{})}),c(M,{type:`NormalBold`,children:d(`timetable.addToTimetable`)})]}))]})]}):c(E,{})})},Ye=d.button`
