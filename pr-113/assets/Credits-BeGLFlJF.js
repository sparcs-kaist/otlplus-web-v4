...
`,ge=({options:e=["option1","option2","option3"],zindex:t=10,disabledOptions:i=[],setSelectedOption:h,selectedOption:c,isDetail:g=!0})=>{const d=q(),[r,n]=o.useState(!1),p=o.useRef(null),[f,b]=o.useState({top:0,left:0,width:0}),[w,S]=o.useState(0);o.useEffect(()=>{if(e.length>0){const l=document.createElement("canvas").getContext("2d");if(!l)return;l.font="14px Pretendard, sans-serif";const m=Math.max(...e.map(v=>l.measureText(v).width));S(m+50)}},[e]),o.useEffect(()=>{if(p.current){const s=p.current.getBoundingClientRect();b({top:s.top,left:s.left,width:Math.max(s.width,w)})}},[r,w]);const x=o.useRef(null);return o.useEffect(()=>{const s=()=>{if(p.current){const l=p.current.getBoundingClientRect();b({top:l.top,left:l.left,width:l.width})}};return s(),window.addEventListener("resize",s),()=>{window.removeEventListener("resize",s)}},[r]),o.useEffect(()=>{if(x.current&&e.length>0&&c>=0){const s=x.current.children[c];if(s){const m=s.getBoundingClientRect().top-x.current.getBoundingClientRect().top;x.current&&(x.current.scrollTop=m)}}},[c,r,e]),I(ue,{isExpand:r,width:w,children:[I(de,{ref:p,onClick:()=>{n(!r)},children:[e[c],u(Z,{onClick:()=>{n(!r)},styles:{padding:0},children:u(V,{size:24,color:d.colors.Highlight.default,onClick:()=>{},children:r?u(re,{}):u(oe,{})})})]}),r&&u(fe,{ref:x,top:f.top,left:f.left,width:f.width,zindex:t,isDetail:g,children:e.map((s,l)=>{const m=i.includes(l),v=c==l;return u(le,{onClick:()=>{h(l),n(!1)},disabled:m,children:u(he,{disabled:m,selected:v,children:s})},l)})})]})},Te=W.memo(ge,(e,t)=>e.options===t.options&&e.zindex===t.zindex&&e.disabledOptions===t.disabledOptions&&e.selectedOption===t.selectedOption&&e.isDetail===t.isDetail);function $e(e,t,i){const{i18n:h}=A(),{infinites:c,headers:g={},enabled:d=!0,staleTime:r=1/0,gcTime:n=300*1e3,initialOffset:p=0,limit:f,select:b=a=>a,iterate:w=()=>-1}=i,[S,x]=o.useState(void 0),[s,l]=o.useState(null),m=o.useRef(b),v=o.useRef(c);o.useEffect(()=>{m.current=b},[b]),o.useEffect(()=>{v.current=c},[c]);const P=Y(e,K(t)),y=te({queryKey:[t,s],queryFn:async({pageParam:a=0})=>{const R=p+a*f,{data:E}=await X.request({method:e,url:"/api/v2"+t,params:{...s,offset:R,limit:f},headers:g});return E},initialPageParam:0,getNextPageParam:(a,R)=>{const E=w(a);if(E!==-1)return E>0?R.length:void 0;for(const B of v.current)if(a[B].length<f)return;return v.current.length>0?R.length:void 0},retry:1,staleTime:r,gcTime:n,enabled:d&&(s!==null||P.safeParse({})?.success===!0)});return o.useEffect(()=>{const a=y.data?.pages;if(!a||a.length===0){x(void 0);return}if(v.current.length===0){const B=a[a.length-1];x(m.current(B));return}const E={...a[a.length-1]};v.current.forEach(B=>{const M=a.flatMap(F=>F[B]);E[B]=M}),x(m.current(E))},[y.data]),o.useEffect(()=>{y.isEnabled&&y.refetch()},[h.resolvedLanguage]),{query:y,setParams:l,data:S}}const k=$(_)`
...
//# sourceMappingURL=Credits-BeGLFlJF.js.map
