...
`,Ee=({options:e=["option1","option2","option3"],zindex:t=10,disabledOptions:c=[],setSelectedOption:h,selectedOption:i,isDetail:g=!0})=>{const d=_(),[r,o]=n.useState(!1),p=n.useRef(null),[f,v]=n.useState({top:0,left:0,width:0}),[w,T]=n.useState(0);n.useEffect(()=>{if(e.length>0){const u=document.createElement("canvas").getContext("2d");if(!u)return;u.font="14px Pretendard, sans-serif";const b=Math.max(...e.map(m=>u.measureText(m).width));T(b+50)}},[e]),n.useEffect(()=>{if(p.current){const s=p.current.getBoundingClientRect();v({top:s.top,left:s.left,width:Math.max(s.width,w)})}},[r,w]);const x=n.useRef(null);return n.useEffect(()=>{const s=()=>{if(p.current){const u=p.current.getBoundingClientRect();v({top:u.top,left:u.left,width:u.width})}};return s(),window.addEventListener("resize",s),()=>{window.removeEventListener("resize",s)}},[r]),n.useEffect(()=>{if(x.current&&e.length>0&&i>=0){const s=x.current.children[i];if(s){const b=s.getBoundingClientRect().top-x.current.getBoundingClientRect().top;x.current&&(x.current.scrollTop=b)}}},[i,r,e]),L(de,{isExpand:r,width:w,children:[L(ae,{ref:p,onClick:()=>{o(!r)},children:[e[i],l(Y,{onClick:()=>{o(!r)},styles:{padding:0},children:l(H,{size:24,color:d.colors.Highlight.default,onClick:()=>{},children:r?l(te,{}):l(re,{})})})]}),r&&l(ue,{ref:x,top:f.top,left:f.left,width:f.width,zindex:t,isDetail:g,children:e.map((s,u)=>{const b=c.includes(u),m=i==u;return l(le,{onClick:()=>{h(u),o(!1)},disabled:b,children:l(fe,{disabled:b,selected:m,children:s})},u)})})]})};function Te(e,t,c){const{i18n:h}=A(),{infinites:i,headers:g={},enabled:d=!0,staleTime:r=1/0,gcTime:o=300*1e3,initialOffset:p=0,limit:f,select:v=a=>a,iterate:w=()=>-1}=c,[T,x]=n.useState(void 0),[s,u]=n.useState(null),b=n.useRef(v),m=n.useRef(i);n.useEffect(()=>{b.current=v},[v]),n.useEffect(()=>{m.current=i},[i]);const P=V(e,K(t)),y=ee({queryKey:[t,s],queryFn:async({pageParam:a=0})=>{const R=p+a*f,{data:E}=await J.request({method:e,url:"/api/v2"+t,params:{...s,offset:R,limit:f},headers:g});return E},initialPageParam:0,getNextPageParam:(a,R)=>{const E=w(a);if(E!==-1)return E>0?R.length:void 0;for(const I of m.current)if(a[I].length<f)return;return m.current.length>0?R.length:void 0},retry:1,staleTime:r,gcTime:o,enabled:d&&(s!==null||P.safeParse({})?.success===!0)});return n.useEffect(()=>{const a=y.data?.pages;if(!a||a.length===0){x(void 0);return}if(m.current.length===0){const I=a[a.length-1];x(b.current(I));return}const E={...a[a.length-1]};m.current.forEach(I=>{const M=a.flatMap(F=>F[I]);E[I]=M}),x(b.current(E))},[y.data]),n.useEffect(()=>{y.isEnabled&&y.refetch()},[h.resolvedLanguage]),{query:y,setParams:u,data:T}}const k=$(W)`
...
//# sourceMappingURL=Credits-DEzHjgjB.js.map
