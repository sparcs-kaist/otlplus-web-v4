...
`,Pi=E.div`
    ...
    user-select: none;
    touch-action: none;
`,Ui=({cellWidth:e=120,fullHeight:t=800.5,lectureSummary:n,setTimeFilter:r,hover:i,setHover:o,selected:a,setSelected:s,removeFunction:u})=>{const{t:h}=Ae(),d=5,f=5,v=8,g=l.useRef(null),R=[h("common.days.monday"),h("common.days.tuesday"),h("common.days.wednesday"),h("common.days.thursday"),h("common.days.friday")];bi(n);const p=32,T=24,m=l.useMemo(()=>t/32,[t]),A=l.useMemo(()=>({"--cell-height":`${m}px`,"--cell-width":`${e}px`,"--col-padding":`${d}px`}),[m,e,d]),[y,M]=l.useState(!1),[S,I]=l.useState(null),[N,w]=l.useState(null),[D,B]=l.useState(null),[k,q]=l.useState(new Map(Array.from({length:f},(F,U)=>[U,Array(p).fill(null)]))),Y=l.useRef(null),V=l.useRef(null),ie=l.useRef(null);Y.current=S,V.current=N,ie.current=D;const G=l.useCallback(F=>{if(g.current&&i==null&&a==null){const U=g.current.getBoundingClientRect(),C=F.clientX-U.left,x=F.clientY-U.top,O=Math.floor(x/m),z=Fn(C,f,[],0,e,d,0);O>=0&&O<p&&z>=0&&z<f&&(M(!0),I(O),w(O),B(z),s(null))}},[i,a,m,p,f,e,d,s]),X=l.useCallback(F=>{if(y&&g.current){const U=g.current.getBoundingClientRect(),C=F.clientY-U.top,x=Math.floor(C/m);x>=0&&x<p&&D!=null&&D>=0&&D<f&&x!==N&&w(x)}},[y,m,p,f,D,N]),K=l.useCallback(()=>{q(new Map(Array.from({length:f},(x,O)=>[O,Array(p).fill(null)])));const F=Y.current,U=V.current,C=ie.current;if(F!=null&&U!=null&&C!=null){const x=Math.min(F,U),O=Math.max(F,U),z={day:C,begin:x*30+480,end:(O+1)*30+480};r(z)}I(null),w(null),M(!1)},[f,p,r]),H=l.useCallback(F=>{if(g.current&&i==null&&a==null){const U=F.touches[0];if(!U)return;const C=g.current.getBoundingClientRect(),x=U.clientX-C.left,O=U.clientY-C.top,z=Math.floor(O/m),_=Fn(x,f,[],0,e,d,0);z>=0&&z<p&&_>=0&&_<f&&(M(!0),I(z),w(z),B(_),s(null))}},[i,a,m,p,f,e,d,s]),oe=l.useCallback(F=>{if(y&&g.current){const U=F.touches[0];if(!U)return;const C=g.current.getBoundingClientRect(),x=U.clientY-C.top,O=Math.floor(x/m);O>=0&&O<p&&D!=null&&D>=0&&D<f&&O!==N&&w(O)}},[y,m,p,f,D,N]),ue=l.useCallback(()=>{K()},[K]);l.useEffect(()=>(y&&window.addEventListener("mouseup",K),()=>{window.removeEventListener("mouseup",K)}),[y,K]);const Z=(F,U,C)=>{const x=new Map(Array.from({length:f},(O,z)=>[z,Array(p).fill(null)]));for(let O=F;O<U+1;O++)x.get(C)[O]=!0;return x};l.useLayoutEffect(()=>{if(g.current&&y){const F=Math.min(S,N),U=Math.max(S,N),C=Z(F,U,D);q(C)}},[N,S,y,D,f,p]);const he=l.useMemo(()=>wi(p,f,e,m,d,[],10,0),[p,f,e,m,d]),we=l.useMemo(()=>xi(n,e,m,d,a,s,i,o,y,u),[n,e,m,d,a,s,i,o,y,u]),Se=l.useCallback(()=>{s(null)},[s]),Q=l.useCallback(F=>{F.stopPropagation(),K()},[K]);return b(Oi,{style:A,children:[c(Bi,{children:Array.from({length:T-v+1},(F,U)=>U+v).map(F=>c(P,{color:"Text.default",children:F%12||12},F))}),b(L,{direction:"column",gap:0,children:[c(L,{direction:"row",gap:5,children:R.map((F,U)=>F!=="none"&&c(zi,{style:F===""?{width:"10px"}:void 0,children:c(P,{color:"Text.default",children:F})},U))}),b(Pi,{ref:g,onClick:Se,onMouseDown:G,onMouseMove:X,onMouseUp:Q,onTouchStart:H,onTouchMove:oe,onTouchEnd:ue,children:[he,Ei(!0,k,"rgba(229, 76, 101, 0.5)",m,e,2,d,[],0,0),we]})]})]})},$n=ne.memo(Ui);function Dr(e){return e.map(t=>t.name).join(", ")}function Fi(e){return e.slice().sort((n,r)=>{const i=n.day,o=r.day;return i!==o?i-o:n.begin-r.begin}).map(n=>n.str).join(" / ")}function $i(e){const t={};return e.forEach(n=>{const r=n.buildingCode,i=n.roomName;r in t?t[r]&&!t[r].includes(i)&&(t[r]=t[r].concat(`, ${i}`)):t[r]=i}),Object.entries(t).map(([n,r])=>`(${n}) ${r}`).join(" / ")}const ji=E(L)`
    width: 100%;
`,Wi=E(L)`
    width: 300px;
    padding: 10px;
...
`,_i=E.a`
    text-decoration: none;
`,Wn=({selectedLecture:e,onMobileModalClose:t,year:n,semester:r,currentTimetableId:i,timetableLectures:o})=>{const a=hn(),s=je(),{t:u}=Ae(),{user:h,status:d}=mn(),f=$e("tablet"),{requestFunction:v}=le("PATCH",`/timetables/${i}`,{onSuccess:()=>{a.invalidateQueries({queryKey:[`/timetables/${i}`]})}}),[g,R]=l.useState([]),{query:p,setParams:T}=le("GET",`/users/${h?.id}/wishlist`,{enabled:d==="success"&&n!=-1});l.useEffect(()=>{n!==-1&&T({year:n,semester:r})},[n,r,i]),l.useEffect(()=>{p.data&&R(p.data.courses.flatMap(I=>I.lectures).map(I=>I.id))},[p.data]);const{requestFunction:m}=le("PATCH",`/users/${h?.id}/wishlist`,{onSuccess:()=>{a.invalidateQueries({queryKey:[`/users/${h?.id}/wishlist`]})}}),A=l.useRef(null),y=I=>{const N={syy:String(n),smtDivCd:String(r),subjtCd:I.code,syllabusOpenYn:"1"};return`https://erp.kaist.ac.kr/com/lgin/SsoCtr/initExtPageWork.do?link=estblSubjt&params=${btoa(JSON.stringify(N))}`},M=async I=>{if(o){if(i){if(o.some(N=>fn(N.classes,I.classes))){alert(u("timetable.addLectureConflict"));return}}else{console.warn("No timetable selected");return}v({action:"add",lectureId:I.id})}},S=async(I,N)=>{d!=="idle"&&m({lectureId:N,mode:I?"delete":"add"})};return c(Ki,{direction:"column",gap:12,align:"center",justify:e?"start":"center",children:e?b(et,{children:[b(Qi,{direction:"column",gap:2,align:"center",justify:"center",children:[b(L,{direction:"row",align:"center",gap:8,justify:f?"space-between":"center",style:{width:"100%"},children:[f&&c("div",{style:{width:20}}),c(P,{type:"Bigger",color:"Text.default",children:e.name}),f&&c(re,{size:20,onClick:t,color:s.colors.Text.default,children:c(Er,{})})]}),c(P,{type:"Big",color:"Text.default",children:e.code})]}),b(L,{direction:"row",gap:8,justify:"flex-end",style:{width:"100%"},children:[c(Zi,{to:`/dictionary?courseId=${e.courseId}`,children:c(P,{type:"Normal",color:"Highlight.default",style:{cursor:"pointer"},children:u("header.dictionary")})}),c(_i,{href:y(e),target:"_blank",children:c(P,{type:"Normal",color:"Highlight.default",style:{cursor:"pointer"},children:u("header.syllabus")})})]}),c(jn,{direction:"column",gap:10,align:"center",children:c(qi,{selectedLecture:e})}),c(jn,{ref:A,direction:"column",gap:10,children:c(Xi,{selectedCourseId:e.courseId,selectedProfessorId:e.professors[0]?.id??-1})}),f&&b(Ji,{direction:"row",gap:12,justify:"flex-end",children:[!g.includes(e.id)&&b(Un,{onClick:()=>{S(g.includes(e.id),e.id),t&&t()},children:[c(re,{size:15,children:c(gn,{})}),c(P,{type:"NormalBold",children:"찜목록에 추가"})]}),!o?.some(I=>I.id===e.id)&&i&&b(Un,{type:"selected",onClick:()=>{M(e),t&&t()},children:[c(re,{size:15,children:c(pn,{})}),c(P,{type:"NormalBold",children:"시간표에 추가"})]})]})]}):c(Ri,{})})},eo=E.div`
    width: 100%;
    display: flex;
    flex-direction: column;
    border-radius: 8px;
